...
function c.splashEnd(f)
  ...
  c.Menu_Difficulty = f.getWidgetN("root/Menu_Difficulty")
  ...
  c.Banner_LivesBonus = f.getWidgetN("root/Game/Hud/Frame/Banner_LivesBonus")
  ...
  c.Menu_StageSelect_Clouds = f.getWidgetN("root/Menu_StageSelect/Frame/Clouds_psys")
  ...
  c.Menu_Continue_Text_Difficulty = f.getWidgetN("root/Menu_Continue/Frame/Text_Difficulty")
  ...
  c.Banner_LivesBonus_Text_Amount = f.getWidgetN("root/Game/Hud/Frame/Banner_LivesBonus/Frame/Container/Text_Life_Bonus_Amount")
  c.Banner_LivesBonus_Text_Lives = f.getWidgetN("root/Game/Hud/Frame/Banner_LivesBonus/Frame/Container/Text_Lives_Amount")
  c.Banner_LivesBonus_Text_Bonus = f.getWidgetN("root/Game/Hud/Frame/Banner_LivesBonus/Frame/Container/Text_Final_Bonus_Amount")
  ...
  -- Gems on the lives bonus screen.
  ---@type {widget: UIWidget, speed: number, delay: number}[]
  c.livesBonusGems = {}
  for i = 0, 15 do
    local name = i == 0 and "Coin" or string.format("Gem_%02d", i) -- Coin and Gem_01 through Gem_15
    table.insert(c.livesBonusGems, {widget = c.Banner_LivesBonus:getChild("Reward_" .. name), speed = 0, delay = math.random() * 5})
  end
  ...
  f.getWidgetN("root/Menu_Difficulty/Frame/Button_Expert"):setCallback("buttonClick", c.difficultySelect, {"expert"})
  f.getWidgetN("root/Menu_Difficulty/Frame/Button_Intermediate"):setCallback("buttonClick", c.difficultySelect, {"intermediate"})
  f.getWidgetN("root/Menu_Difficulty/Frame/Button_Beginner"):setCallback("buttonClick", c.difficultySelect, {"beginner"})
  f.getWidgetN("root/Menu_Difficulty/Frame/Button_Cancel"):setCallback("buttonClick", c.difficultyCancel)
  ...
  f.getWidgetN("root/Game/Hud/Frame/Banner_LivesBonus/Frame/Button_Ok"):setCallback("buttonClick", c.livesBonusOk)
...
function c.gameWinOk(f)
  -- First check if we've got any lives remaining.
  if f.profileGetLives() > 0 then
    -- If so, display the life bonus.
    c.Banner_LivesBonus:show()
    c.Banner_LivesBonus:showParticles()
    c.Banner_LivesBonus:setActive()
  else
    -- Otherwise, check if we've got a highscore.
...
-- WHEN CLICKED DONE ON LIVES BONUS SCREEN
function c.livesBonusOk(f)
  c.Banner_LivesBonus:hide()
  c.Banner_LivesBonus:hideParticles()
  c.Banner_LivesBonus:scheduleFunction("hideEnd", function()
    -- Check if we've got a highscore.
    local place = f.profileHighscoreWrite()
    if place then
      c.highscorePlace = place
      c.Banner_HighScore:show()
...
function c.mainStart(f)
  if f.profileGetSession() then
    ...
    c.Menu_Difficulty:show()
    c.Menu_Difficulty:setActive()
  end
end
...
-- WHEN CLICKED A DIFFICULTY BUTTON ON DIFFICULTY MENU
function c.difficultySelect(f, params)
  local difficulty = params[1]
  c.newGameDifficulty = difficulty
  c.Menu_Difficulty:hide()
  c.Menu_Difficulty:scheduleFunction("hideEnd", c.stageSelectShow)
end

-- WHEN CLICKED CANCEL ON DIFFICULTY MENU
function c.difficultyCancel(f)
  c.Menu_Difficulty:hide()
  c.Main:setActive()
end
...
function c.mainMapStart(f)
  f.resetActive()
  f.profileNewGame(c.newGameStage, "difficulties/" .. c.newGameDifficulty .. ".json")
...
function c.tick(f, params)
  ...
  -- Update life bonus particles
  if c.Banner_LivesBonus and c.Banner_LivesBonus:isVisible() then
    for i, gem in ipairs(c.livesBonusGems) do
      if gem.delay > 0 then
        gem.delay = gem.delay - dt
        if gem.delay <= 0 then
          gem.widget.x = math.random() * 800
          gem.widget.y = -100
          gem.speed = 100
        end
      else
        gem.speed = gem.speed + 250 * dt
        gem.widget.y = gem.widget.y + gem.speed * dt
        if gem.widget.y >= 700 then
          gem.delay = math.random() * 5
        end
      end
    end
  end
...
function c.levelComplete(f)
  ...
  -- Life bonus!
  if f.profileGetLevel() == f.configGetLevelCount("level_sets/adventure.json") then
