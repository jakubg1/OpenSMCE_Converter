...
def convert_level(contents):
	level_data = {
		...
		"music": "music_playlists/music_level.json",
		"dangerMusic": "music_playlists/music_danger.json",
...
def convert_psys(contents):
	...
	for words in preprocess_contents(contents):
		...
		if words[0] == "AnimRate":
			rate = float(words[2])
			spawner_data["particleData"]["animationSpeed"] = (1000 / rate) if rate > 0 else 0
